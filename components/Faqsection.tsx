...
export default function Faqsection() {
  ...
  return (
    ...
    >
      ...
      {/* Heading */}
      <h1
        className={`font-custom1 text-white text-[80px] sm:text-[70px] md:text-[90px] lg:text-[130px] mb-8 ${
      ...
      <div className="w-full max-w-md sm:max-w-lg lg:max-w-2xl flex flex-col">
        <div className="max-h-96 overflow-y-auto bg-[#FBAA4B] p-4 rounded-xl shadow-lg custom-scrollbar">
          {[
            ...
            <div 
            key={index}
            className={`p-4 font-custom2 rounded-xl shadow-md cursor-pointer transition-all duration-500 ease-in-out mb-4 text-3xl ${
              selectedIndex === index 
      ...
      {/* Answer Box */}
      <div
  className={`relative w-full max-w-md font-custom2 sm:max-w-lg lg:max-w-2xl bg-[#FBAA4B] text-[#5C3317] font-medium p-6 rounded-lg shadow-md mt-8 text-center text-2xl sm:text-3xl md:text-4xl transition-all duration-500 ease-in-out transform ${
    selectedAnswer ? "opacity-100 scale-100" : "opacity-0 scale-75"
  }`}
>
  {/* "Answer" in the top-left corner */}
  <div className="absolute top-2 left-4 text-lg sm:text-xl md:text-2xl font-bold text-[#5C3317]">
    Answer
  </div>
